...
export interface ConversationBarProps {
  /**
   * ElevenLabs Agent ID to connect to when using a public agent.
   */
  agentId?: string

  /**
   * Returns a signed ElevenLabs conversation URL from a trusted server route.
   */
   ...
   */
  onStatusChange?: (status: ConversationBarStatus) => void

  /**
   * Callback when the agent mode changes
   */
...
>(
  (
    ...
    const [agentState, setAgentState] =
      React.useState<ConversationBarStatus>("disconnected")
    ...
    const startConversation = React.useCallback(async () => {
      try {
        ...
        if (!agentId) {
          throw new Error("agentId or getSignedUrl is required")
        }
    ...
    }, [conversation, getMicStream, getSignedUrl, agentId, onError, onStatusChange])
    ...
    const handleStartOrEnd = React.useCallback(() => {
      if (agentState === "connected" || agentState === "connecting") {
        handleEndSession()
      } else if (agentState === "disconnected" || agentState === "error") {
        startConversation()
      }
